...
def setup_mnist(data_pipeline):

    def normalize_img(image):
        """Normalizes images: `uint8` -> `float32`."""
        return tf.cast(image, tf.float32) / 255.
    data_pipeline = data_pipeline.map(normalize_img,
...
def setup_cifar10(data_pipeline):
...
def setup_inception_v1(data_pipeline):
    model = inception_v1()
    ...
    def transform_images(image, new_size):
        ...
        return img
    data_pipeline = data_pipeline.map(lambda row: transform_images(row, 224),
                                      num_parallel_calls=tf.data.AUTOTUNE)
    return model, data_pipeline


def setup_inception_v3(data_pipeline):
    def transform_images(image, new_size):
        return tf.keras.applications.inception_v3.preprocess_input(
            tf.keras.layers.Resizing(new_size, new_size)(image))
    data_pipeline = data_pipeline.map(lambda row: transform_images(row, 299),
                                      num_parallel_calls=tf.data.AUTOTUNE)
